...
impl<H: HashFunction> BinaryEvaluator<H> {
    ...
    pub fn new(decoding_infos: HashMap<usize, u8>, hash: H, gc: Vec<Block>) -> BinaryEvaluator<H> {
        BinaryEvaluator {
            decoding_infos,
            hash,
            cache: gc,
...
impl<H: HashFunction> ExecutionPrimitives for BinaryEvaluator<H> {
    ...
    /// # Arguments
    ///
    /// * `_id` - The identifier for the evaluator input.
    /// * `_x` - The value provided as input.
    ///
    /// # Returns
    ///
    /// A result containing
