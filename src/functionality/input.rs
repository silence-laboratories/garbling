    ...
    H: HashFunction,
{
    ...
    if pid == 0 || pid == 1 {
        if party_id == 0 || party_id == 1 {
            ...
            if party_id == pid {
                let mut send2 = [0u8; 64];
                if input.unwrap() {
                    send2[0..32].copy_from_slice(&w1);
                    send2[32..64].copy_from_slice(&wit1);
                } else {
                    send2[0..32].copy_from_slice(&w0);
                    send2[32..64].copy_from_slice(&wit0);
                }

                send_to_party(setup, mpc_encryption, tag2, send2, 2, relay).await?;
            }
            ...
            let coms: Vec<[u8; 64]> = receive_from_parties(
                setup,
                mpc_encryption,
                tag1,
                2 * Block::default().external_size(),
                vec![0, 1],
                relay,
            )
            .await?;
            assert_eq!(coms[0], coms[1]);

            let mut com0 = Block::default();
            com0.copy_from_slice(&coms[0][0..32]);
            let mut com1 = Block::default();
            com1.copy_from_slice(&coms[0][32..64]);

            let decom: Vec<[u8; 64]> = receive_from_parties(
                setup,
                mpc_encryption,
                tag2,
                2 * Block::default().external_size(),
            ...
            msg.copy_from_slice(&decom[0][0..32]);
            let mut wit = Block::default();
            wit.copy_from_slice(&decom[0][32..64]);
