...
pub fn build_zcash_import_function() -> BinaryCircuit {
    ...
    let mut prime_bytes = hex::decode(Scalar::MODULUS).unwrap();
    prime_bytes.reverse();
...
fn build_mod_add_circut(size: usize, prime_bytes: [u8; 32]) -> BinaryCircuit {
    let mut builder = CircuitBuilder::new();

    let mut pbin = BinaryString {
        length: size as u64,
        value: prime_bytes.to_vec(),
    };
